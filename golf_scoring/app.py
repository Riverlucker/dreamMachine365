...
if page == "🏆 Leaderboard":
    ...
    if not tournament.get("players"):
        ...
        if is_ryder_cup:
            tab_ryder_cup, tab_matchplay = st.tabs([
                "🏆 Ryder Cup Scoreboard",
                "⚔️ Matchplay Duelle"
            ])
            tab_team, tab_player_net, tab_player_gross, tab_winners = None, None, None, None
        elif has_matchplay:
            tab_team, tab_player_net, tab_player_gross, tab_matchplay, tab_winners = st.tabs([
                "👥 Team Netto", 
                "🏌️ Spieler Netto", 
                "🏌️ Spieler Brutto",
                "⚔️ Matchplay Duelle",
            ...
            tab_team, tab_player_net, tab_player_gross, tab_winners = st.tabs([
                "👥 Team Netto", 
                "🏌️ Spieler Netto", 
                "🏌️ Spieler Brutto",
                "🏆 Previous Winners"
            ])
            tab_ryder_cup, tab_matchplay = None, None
        ...
        if not is_ryder_cup and tab_team:
            with tab_team:
                ...
                # Sort teams by diff asc
                team_table_data.sort(key=lambda x: x["diff"])
                
                # Render Team Table
                html = "<div class='leaderboard-container'><table class='leaderboard-table'>"
                html += "<thead><tr><th>Rang</th><th>Team</th><th>Netto Par-Diff</th><th>Gespielte Löcher</th></tr></thead><tbody>"
                for i, team_item in enumerate(team_table_data):
                    ...
                    html += "<tr>"
                    html += f"<td><span class='rank-badge {rank_class}'>{i+1}</span></td>"
                    html += f"<td><span class='team-tag' style='background-color:{team_item['color']}'>{team_item['name']}</span></td>"
                    html += f"<td><strong>{diff_str}</strong> <span style='font-size:0.8rem;color:#888'>({team_item['points']} Pkt)</span></td>"
                    html += f"<td>{team_item['holes']}</td>"
                    html += "</tr>"
                html += "</tbody></table></div>"
                st.markdown(html, unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
                        ...
                        for p in team_item["players"]:
                            ...
                            if is_overall:
                                for r in rounds_list:
                                    ...
                                    html_details += f"<td>{pts} <span style='font-size:0.75rem; color:#888;'>({holes}L)</span></td>"
                                html_details += f"<td><strong>{p['total_net_points']}</strong></td>"
                            else:
                                r_stats = p["rounds"].get(active_round["id"], {})
                                pts = r_stats.get("net_points", 0)
                                holes = r_stats.get("holes_played", 0)
                                html_details += f"<td><strong>{pts}</strong> <span style='font-size:0.75rem; color:#888;'>({holes}L)</span></td>"
                            html_details += "</tr>"
        ...
        if not is_ryder_cup and tab_player_net:
            with tab_player_net:
                ...
                if is_overall:
                    # Sort overall net points desc
                    player_list.sort(key=lambda x: x["total_net_points"], reverse=True)
                else:
                    # Sort single round net points desc
                    player_list.sort(key=lambda x: x["rounds"].get(active_round["id"], {}).get("net_points", 0), reverse=True)
                ...
                if is_overall:
                    ...
                    html += "<th>Gesamt</th><th>Netto Par-Diff</th></tr></thead><tbody>"
                else:
                    html += f"<th>Punkte</th><th>Netto Par-Diff</th><th>Löcher</th></tr></thead><tbody>"
                ...
                for i, p in enumerate(player_list):
                    ...
                    if is_overall:
                        for r in rounds_list:
                            ...
                            html += f"<td>{pts} <span style='font-size:0.75rem; color:#888;'>({h}L)</span></td>"
                        html += f"<td><strong>{total_pts}</strong></td>"
                    else:
                        html += f"<td><strong>{total_pts}</strong></td>"
                        
                    html += f"<td>{diff_str}</td>"
                    if not is_overall:
                        html += f"<td>{holes_played}</td>"
                    html += "</tr>"
        ...
        if not is_ryder_cup and tab_player_gross:
            with tab_player_gross:
                ...
                if is_overall:
                    player_list.sort(key=lambda x: x["total_gross_points"], reverse=True)
                else:
                    player_list.sort(key=lambda x: x["rounds"].get(active_round["id"], {}).get("gross_points", 0), reverse=True)
                ...
                if is_overall:
                    for r in rounds_list:
                        html_details_name = r['name'].replace("Runde ", "R").split(" - ")[0]
                        html += f"<th>{html_details_name}</th>"
                    html += "<th>Gesamt</th><th>Brutto Par-Diff</th></tr></thead><tbody>"
                else:
                    html += f"<th>Punkte</th><th>Brutto Par-Diff</th><th>Löcher</th></tr></thead><tbody>"
                    ...
                    for m in m_list:
                        ...
                        with st.expander(f"Lochkarte: {m['name']} ({status['result_text']})"):
                            ...
                            for hd in status["hole_details"]:
                                ...
                                html_card += "</tr>"
                            html_card += "</tbody></table></div>"
                            st.markdown(html_card, unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
                
        # 🏆 Previous Winners Tab
        if not is_ryder_cup and tab_winners:
            with tab_winners:
                st.markdown("<div class='golf-card'>", unsafe_allow_html=True)
    ...
    if check_password():
        ...
        if not tournament.get("rounds"):
            ...
            if not is_ryder_cup:
                ...
                st.session_state.selected_players = selected_player_names
                ...
                if is_ryder_cup:
                    ...
                    for m in matchups:
                        ...
                        with c1:
                            if st.button(f"{name_a} gewinnt", key=f"win_a_{m_id}_{selected_hole}", use_container_width=True):
                                data_manager.update_scores_bulk(event_id, active_round["id"], selected_hole, {p_a_id: 1, p_b_id: 2})
                                st.rerun()
                        with c2:
                            if st.button("Geteilt", key=f"tie_{m_id}_{selected_hole}", use_container_width=True):
                                data_manager.update_scores_bulk(event_id, active_round["id"], selected_hole, {p_a_id: 1, p_b_id: 1})
                                st.rerun()
                        with c3:
                            if st.button(f"{name_b} gewinnt", key=f"win_b_{m_id}_{selected_hole}", use_container_width=True):
                                data_manager.update_scores_bulk(event_id, active_round["id"], selected_hole, {p_a_id: 2, p_b_id: 1})
                                st.rerun()
                        with c4:
                            if st.button("Löschen", key=f"clear_{m_id}_{selected_hole}", use_container_width=True):
                                data_manager.update_scores_bulk(event_id, active_round["id"], selected_hole, {p_a_id: 0, p_b_id: 0})
                                st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    if not filtered_players:
                        ...
                        for p in filtered_players:
                            ...
                            hcp_strokes = scoring.get_hole_handicap_strokes(playing_hcp, hole_info["index"], len(course["holes"]))
                            ...
                            with c_minus:
                                if st.button("➖", key=f"minus_{p_id}_{selected_hole}", use_container_width=True):
                                    ...
                                    data_manager.update_scores_bulk(event_id, active_round["id"], selected_hole, {p_id: new_val})
                                    st.rerun()
                            with c_score:
                                st.markdown(f"<div style='text-align:center; font-size:1.5rem; font-weight:bold; margin-top:5px;'>{existing_val if existing_val > 0 else '-'}</div>", unsafe_allow_html=True)
                            with c_plus:
...
# ----------------- ⚙️ ADMIN PAGE -----------------
    ...
    if check_password("admin"):
        ...
        admin_tab_backup, admin_tab_config = st.tabs([
